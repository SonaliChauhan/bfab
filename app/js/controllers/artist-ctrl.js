    ...
    $scope.$watch('search', function(value) {
        ...
        if(!completeList.length)
            ...
            completeList.forEach(function(column) {
                var found = column.first_name.toLowerCase().search(value)>-1 ||column.last_name.toLowerCase().search(value)>-1 || column.email.search(value)>-1;
                found = found || column.gender.toLowerCase().search(value)>-1 || column.registration_datetime.toLowerCase().search(value)>-1;
                found = found || column.city.toLowerCase().search(value)>-1;
                if(found)
...
App.controller('NewRequestedController', function ($scope, $http, $location, $cookies, $cookieStore, MY_CONSTANT, $timeout, $window, ngDialog) {
    ...
    $scope.getArtistList = function (page_number) {
        ...
        $scope.page_no = page_number;
        var index = (page_number - 1) * $scope.page_length;
        $.post(MY_CONSTANT.url + '/new_requests_paging', {
            access_token: $cookieStore.get('obj').accesstoken,
            page_no: page_number,
            length: $scope.page_length
        }, function (data) {
...
// ****Search in Artist list***//
    ...
    $scope.$watch('search', function (value) {
        ...
        if (!completeList.length)
            ...
            completeList.forEach(function (column) {
                var found = column.first_name.toLowerCase().search(value) > -1 || column.last_name.toLowerCase().search(value) > -1 || column.email.search(value) > -1;
                found = found || column.gender.toLowerCase().search(value) > -1 || column.registration_datetime.toLowerCase().search(value)>-1 || column.city.toLowerCase().search(value) >-1;
                if (found)
                    putData(column, searchArray, ++index);
            });
            $scope.searchResults(0, $scope.page_length, $scope.page_no);
